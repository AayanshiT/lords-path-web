...
export default function HealthInfoSection() {
  return (
    <section className="w-full px-4 md:px-10 py-10 bg-white">
      ...
      {/* Trust Bar */}
      <div className="rounded-2xl bg-cyan-50 border border-cyan-100 px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="text-center md:text-left">
          <h3 className="text-xl font-semibold text-gray-900">
            Why <span className="text-teal-600">8.5 Million Indians</span>
          </h3>
          <p className="text-gray-700 font-medium">
            Trust Healthians Labs
          </p>
        </div>

        <div className="flex flex-wrap justify-center gap-6 text-gray-700">
          <Feature icon={<CheckCircle />} text="CAP & NABL Accredited Labs" />
          <Feature icon={<Clock />} text="On Time Sample Collection" />
          <Feature icon={<FileText />} text="Smart Reports in 6 Hours" />
          <Feature icon={<Headphones />} text="Free Report Consultation" />
        </div>
      </div>
    </section>
  );
